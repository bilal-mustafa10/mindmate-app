...
type Props = {
    navigation: NativeStackNavigationProp<RootStackParamList, 'DailyResource'>;
};

export default function DailyResourceScreen({ navigation }: Props) {
    const { results } = useSelector((state: RootState) => state.resources);
    const dailyBoost = results.find((resource) => resource.daily_boost === true);

    const content = JSON.parse(dailyBoost.content);
